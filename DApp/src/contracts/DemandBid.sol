...
contract DemandBid {
    ...
    // uint256 is the date
    mapping (address => mapping (uint => Bet_Info)) public agent_details;
    mapping (uint => Round) public round_info;
    ...
    struct Round {
        ...
        uint winnerPrediction; //add for webapp
    }
  ...
  //for demostration, mock the settlement values
  function mock() public {
      //uint totalPots[7]= [13,15,32,25,40,45,35];
      //uint settlementValues[7] = [20,10,30,40,35,32,33];

      while(currentDay < 10) {
      //set prediction
      agent_details[msg.sender][currentDay].prediction = 9 + currentDay;
      //set bet amount
      agent_details[msg.sender][currentDay].betAmount = 100;
      //set total pot
      round_info[(currentDay)].total_pot = 100;
      //set settlement value
      setSettlementValue(100);
      currentDay += 1;
      }
  }

  function getBetAmount(uint day) public returns(uint) {
    return agent_details[msg.sender][day].betAmount;
  }
